...
export default function NavEl(props) {
	...
	const navOnClicks = {
		...
		products: () => {
			dispatch(setPageMethods.allProducts());
			mobileNavToggle();
		},
		cart: () => {
			dispatch(setPageMethods.cart());
			mobileNavToggle();
		},
		addProducts: () => {
			dispatch(setPageMethods.addProduct());
			mobileNavToggle();
		},
		login: () => {
			dispatch(setPageMethods.login());
			mobileNavToggle();
		},
		signUp: () => {
			dispatch(setPageMethods.signUp());
			mobileNavToggle();
		},
	};
	...
	const basicNavRoutes = [
		...
		{ name: "Cart", ref: "/#Cart", onClick: navOnClicks.cart },
		{
			name: "Add Product",
			ref: "/#Add-Product",
			onClick: navOnClicks.addProducts,
		},
	];
	...
	const loggedInNavElements = [{ name: "Log Out", onClick: logout }];
	...
	const desktopBasicNavRoutes = basicNavRoutes.filter(
	...
	const BasicNavRoutes = () =>
		isMobileNav == true
			? makeNavElements(basicNavRoutes)
			: isMobileNav == false
			? makeNavElements(desktopBasicNavRoutes)
			: "";
